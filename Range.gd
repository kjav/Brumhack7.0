...
func setupBlocks():
	if (typeof(maxProgress) == typeof(0)) and (typeof(progress) == typeof(0)):
		...
		getPlusButton().set_pos(Vector2(width / 2 - (button_size - 40), -60))
		# Set position of minus button
		getMinusButton().set_pos(Vector2(-width / 2 - (button_size - 80), -60))
		# Set position of value label
		getValueLabel().set_pos(Vector2(width / 2 + 10, -21))
		# Set position of name label
		getNameLabel().set_pos(Vector2(-width / 2 - 290, -21))
		for i in range(blocks.size()):
			blocks[i].set_opacity(0)
			blocks[i].free()
		blocks = []
		for i in range(maxProgress):
			...
			new_block.set_pos(Vector2(-width / 2 + i * 30 + 50, -20))
